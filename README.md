...
**方式 B：GitHub 网页 Download ZIP** —— 注意：**macOS 解压后文件会丢掉执行位**，双击会报"没有正确的访问权限"。下载完先在终端跑一次：

```bash
cd ~/Downloads/macSystemCleaner-main   # 路径按你解压的位置调整
chmod +x *.command
```

或者，**完全跳过执行位问题**：直接用 `zsh` 指令跑脚本（zsh 直接读文本内容，不需要执行位）。下面所有命令都是这种写法。

### 第一步：先诊断（强烈建议）

```bash
zsh verify_all.command
# 或者（若已 chmod +x）双击 verify_all.command
```
...
# 或者（若已 chmod +x）双击 MacSystemCleaner.command
```
