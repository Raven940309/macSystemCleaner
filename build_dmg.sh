...
▶ 第二步（可选）：双击「2-系统数据清理.command」
  ...
  每一项的确认提示有三个选项：
    y — 确认清理
    n — 跳过
    o — 打开 Finder 自己看（工具不动数据，你在 Finder 里自己决定）
...
▶ 清理后反悔了？—— 回滚（v0.7 新增）
  ...
  「移到 .bak 备份」这一档（上面第 2 种）的清理都可以回滚。
  下次双击「2-系统数据清理.command」，在主菜单输入 r 进入回滚界面：
    • 查看所有历史清理记录（时间 / 项目 / 备份大小 / 状态）
    • 单项或批量回滚（把 .bak 重新放回原位）
    • 不再需要时，也可以一次性删除所有备份腾出空间
  备份总占用超过 3 GB 时，启动菜单会醒目提示你。
